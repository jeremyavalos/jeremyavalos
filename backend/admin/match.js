(() => {
  ...
  let selected = null; let legal = []; let currentGame = null;
  ...
  function renderBoard() {
    const board=document.getElementById('board'); board.replaceChildren(); const map=position(currentGame.fen_current); const adminColor=currentGame.challenger_color==='white'?'black':'white'; const whiteBottom=adminColor==='white'; const ranks=whiteBottom?[8,7,6,5,4,3,2,1]:[1,2,3,4,5,6,7,8]; const files=whiteBottom?'abcdefgh'.split(''):'hgfedcba'.split('');
