(() => {
  ...
  async function showVisitor(ip) {
    ...
    try {
      ...
      [['IP',v.ip],['Country / region',v.country || 'Unknown'],['Browser',v.browser_family || 'Unknown'],['Device',v.device_category || 'Unknown'],['First seen',new Date(v.first_seen).toLocaleString()],['Last seen',new Date(v.last_seen).toLocaleString()],['Total visits',v.total_visits]].forEach(([label,value]) => { const item = el('div'); item.append(el('label','',label), el('span','',value)); grid.append(item); });
      const visits = el('ul','visit-list'); v.recent_visits.forEach(visit => { const li = el('li'); li.append(el('div','',visit.path || '/'), el('div','state',`${new Date(visit.created_at).toLocaleString()} · ${visit.referrer || 'Direct'}`)); visits.append(li); }); box.append(grid, el('h3','','RECENT ACTIVITY'), visits); target.replaceChildren(box);
  ...
  async function loadVisitors(page = visitorPage) {
    ...
    try {
      ...
      const table = el('table'); const head = el('thead'); const hr = el('tr'); ['IP','COUNTRY','DEVICE','VISITS','LAST SEEN'].forEach(x => hr.append(el('th','',x))); head.append(hr); const body = el('tbody');
      visitors.forEach(v => { const row = el('tr'); [v.ip,v.country || 'Unknown',v.device_category || 'Unknown',v.visits,relative(v.last_seen)].forEach((x,i) => row.append(el('td',i === 2 ? 'hide-mobile' : '',x))); row.addEventListener('click',() => showVisitor(v.ip)); body.append(row); }); table.append(head,body); document.getElementById('visitors-content').replaceChildren(table);
