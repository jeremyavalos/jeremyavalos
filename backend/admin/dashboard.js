(() => {
  ...
  const trackingText = event => [['ref',event.ref],['source',event.utm_source],['medium',event.utm_medium],['campaign',event.utm_campaign],['content',event.utm_content],['term',event.utm_term]].filter(([,value])=>value).map(([key,value])=>`${key}: ${value}`).join(' · ') || '—';
  ...
  async function showVisitor(visitorId) {
    ...
    try {
      ...
      const acquisition=el('div','detail-grid'); const a=v.acquisition; [['First source',a?.source||'—'],['First medium',a?.medium||'—'],['First campaign',a?.campaign||'—'],['First ref',a?.ref||'—'],['First referrer',a?referrerText(a):'Direct'],['First landing path',a?.path||'—'],['First seen',a?.created_at?new Date(a.created_at).toLocaleString():'—']].forEach(([label,value])=>{const item=el('div');item.append(el('label','',label),el('span','',value));acquisition.append(item);});
      ...
      const visits = el('ul','visit-list'); v.recent_visits.forEach(visit => { const li = el('li'); li.append(el('div','',`${visit.path || '/'} · ${visit.ip || 'IP unavailable'}`), el('div','state',`${new Date(visit.created_at).toLocaleString()} · ${referrerText(visit)} · ${trackingText(visit)} · ${visit.browser_family || 'Unknown browser'} / ${visit.device_category || 'Unknown device'}`)); visits.append(li); });
      box.append(grid, el('h3','detail-heading','ACQUISITION'), acquisition, el('p','state','First known acquisition is the earliest retained event with tracking or a referrer; later direct visits do not replace it.'), el('h3','','ASSOCIATED CHALLENGES'), associationNote, associationContent, el('h3','','OBSERVED IPS / APPROXIMATE LOCATIONS'), ips, el('h3','','RECENT TRACKED VISITS / EVENTS'), visits); target.replaceChildren(box);
  ...
  async function showIp(ip, historyPage = 1) {
    ...
    try {
      ...
      const history=el('table','history-table'); const hh=el('tr'); ['TIMESTAMP','VISITOR ID','GAMERTAG','PATH','REFERRER','TRACKING','DEVICE','BROWSER','COUNTRY','REGION','CITY','NETWORK'].forEach(x=>hh.append(el('th','',x))); const hb=el('tbody'); detail.events.forEach(event=>{const row=el('tr');const visitorCell=el('td');if(event.visitor_id){const link=el('button','text-link',shortVisitorId(event.visitor_id));link.title=event.visitor_id;link.addEventListener('click',()=>{document.getElementById('visitors').scrollIntoView();showVisitor(event.visitor_id);});visitorCell.append(link);}else visitorCell.textContent='Legacy / unidentified';[new Date(event.created_at).toLocaleString(),visitorCell,(event.associated_gamertags||[]).join(', ')||'—',event.path||'/',referrerText(event),trackingText(event),event.device_category||'Unknown',event.browser_family||'Unknown',event.country||'Unknown',event.region||'Unknown',event.city||'Unknown',network(event.asn_org)].forEach((value,index)=>row.append(index===1?value:el('td','',value)));hb.append(row);});history.append(el('thead','').appendChild(hh).parentNode,hb);
  ...
  async function showTracking(ref, page=1) {
    ...
    try { const { referral, pagination }=await api(`/api/admin/tracking/${encodeURIComponent(ref)}?page=${page}&page_size=25`);const box=el('div','detail');box.append(el('h3','detail-title',`REF ${ref}`),el('p','state','Visitor IDs and IPs are analytics attributes, not proof of a real person’s identity.'));
      ...
      const events=el('table','history-table');const eh=el('tr');['TIMESTAMP','VISITOR ID','IP','PATH','REFERRER','SOURCE','MEDIUM','CAMPAIGN','CONTENT','TERM'].forEach(x=>eh.append(el('th','',x)));const eb=el('tbody');referral.events.forEach(event=>{const row=el('tr');[new Date(event.created_at).toLocaleString(),event.visitor_id?shortVisitorId(event.visitor_id):'Legacy / unidentified',event.ip||'—',event.path||'/',referrerText(event),event.utm_source||'—',event.utm_medium||'—',event.utm_campaign||'—',event.utm_content||'—',event.utm_term||'—'].forEach(value=>row.append(el('td','',value)));eb.append(row);});events.append(el('thead','').appendChild(eh).parentNode,eb);const pager=el('div','pagination');if(page>1){const prev=el('button','','← NEWER');prev.addEventListener('click',()=>showTracking(ref,page-1));pager.append(prev);}pager.append(el('span','state',`PAGE ${pagination.page} / ${Math.max(1,pagination.pages)}`));if(page<pagination.pages){const next=el('button','','OLDER →');next.addEventListener('click',()=>showTracking(ref,page+1));pager.append(next);}box.append(el('h3','detail-heading','ASSOCIATED VISITOR IDs'),visitors,el('h3','detail-heading','EVENT HISTORY'),events,pager);target.replaceChildren(box);target.scrollIntoView({block:'start'});
    } catch(error){target.replaceChildren(el('div','error',`Unable to load referral — ${error.message}`));}
  }
  async function loadTracking(page=trackingPage){trackingPage=page;try{const { tracking,pagination }=await api(`/api/admin/tracking?page=${page}&page_size=25`);if(!tracking.length){state('tracking-content','No tracked links recorded yet.');return;}const table=el('table');const head=el('tr');['REF','SOURCE','MEDIUM','CAMPAIGN','VISITS / EVENTS','VISITOR IDS','FIRST SEEN','LAST SEEN'].forEach(x=>head.append(el('th','',x)));const body=el('tbody');tracking.forEach(item=>{const row=el('tr');const refCell=el('td');if(item.ref){const link=el('button','text-link',item.ref);link.addEventListener('click',()=>showTracking(item.ref));refCell.append(link);}else refCell.textContent='—';row.append(refCell,el('td','',item.source||'—'),el('td','',item.medium||'—'),el('td','',item.campaign||'—'),el('td','',item.visits),el('td','',item.distinct_visitor_ids),el('td','',new Date(item.first_seen).toLocaleString()),el('td','',new Date(item.last_seen).toLocaleString()));body.append(row);});table.append(el('thead','').appendChild(head).parentNode,body);document.getElementById('tracking-content').replaceChildren(table);const pager=document.getElementById('tracking-pagination');pager.replaceChildren();if(page>1){const prev=el('button','','← PREV');prev.addEventListener('click',()=>loadTracking(page-1));pager.append(prev);}pager.append(el('span','state',`PAGE ${pagination.page} / ${Math.max(1,pagination.pages)}`));if(page<pagination.pages){const next=el('button','','NEXT →');next.addEventListener('click',()=>loadTracking(page+1));pager.append(next);}}catch(error){state('tracking-content',`Unable to load tracking — ${error.message}`,true);}}
