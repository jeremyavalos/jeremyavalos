(() => {
  ...
  async function loadVisitors(page = visitorPage) {
    ...
    try {
      ...
      const table = el('table'); const head = el('thead'); const hr = el('tr'); ['IP','GAMERTAG','COUNTRY','REGION','CITY','DEVICE','VISITS','LAST SEEN'].forEach(x => hr.append(el('th','',x))); head.append(hr); const body = el('tbody');
      visitors.forEach(v => { const row = el('tr'); const gamertags=(v.associated_gamertags || []).join(', ') || '—'; [[v.ip,''],[gamertags,''],[v.country || 'Unknown','hide-mobile'],[v.region || 'Unknown','hide-mobile'],[v.city || 'Unknown','hide-mobile'],[v.device_category || 'Unknown','hide-mobile'],[v.visits,''],[relative(v.last_seen),'']].forEach(([value,className]) => row.append(el('td',className,value))); row.addEventListener('click',() => showVisitor(v.ip)); body.append(row); }); table.append(head,body); document.getElementById('visitors-content').replaceChildren(table);
