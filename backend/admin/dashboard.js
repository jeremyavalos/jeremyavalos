(() => {
  ...
  async function loadOverview() {
    try {
      ...
      document.getElementById('overview-content').replaceChildren(metric('Total site visits', o.total_visits), metric('Visits today', o.visits_today), metric('Unique visitors', o.unique_visitors), metric('Challenges created', o.total_challenges), metric('Active matches', o.active_matches), metric('Games played', o.games_completed), metric('Wins', o.jeremy_wins), metric('Losses', o.player_wins), metric('Current streak', streak), metric("Jeremy's move", o.my_turn));
  ...
  async function showVisitor(ip) {
    ...
    try {
      const { visitor: v } = await api(`/api/admin/visitors/${encodeURIComponent(ip)}`); const box = el('div', 'detail'); const grid = el('div', 'detail-grid');
      [['IP',v.ip],['Associated gamertags',(v.associated_gamertags || []).join(', ') || '—'],['Country (approx.)',v.country || 'Unknown'],['Region (approx.)',v.region || 'Unknown'],['City (approx.)',v.city || 'Unknown'],['Timezone',v.timezone || 'Unknown'],['ASN / ISP',v.asn_org || 'Unknown'],['Browser',v.browser_family || 'Unknown'],['Device',v.device_category || 'Unknown'],['First seen',new Date(v.first_seen).toLocaleString()],['Last seen',new Date(v.last_seen).toLocaleString()],['Total visits',v.total_visits]].forEach(([label,value]) => { const item = el('div'); item.append(el('label','',label), el('span','',value)); grid.append(item); });
      ...
      const associationNote = el('p','state','Location is approximate. Associated gamertags are based on the challenge creation IP; an IP address is not a reliable identity.');
      const associationContent = associations.childElementCount ? associations : el('div','empty','No associated challenges.');
      const visits = el('ul','visit-list'); v.recent_visits.forEach(visit => { const li = el('li'); li.append(el('div','',visit.path || '/'), el('div','state',`${new Date(visit.created_at).toLocaleString()} · ${visit.referrer || 'Direct'}`)); visits.append(li); });
      box.append(grid, el('h3','','ASSOCIATED CHALLENGES'), associationNote, associationContent, el('h3','','RECENT ACTIVITY'), visits); target.replaceChildren(box);
  ...
  async function loadVisitors(page = visitorPage) {
    ...
    try {
      ...
      const table = el('table'); const head = el('thead'); const hr = el('tr'); ['IP','GAMERTAG','COUNTRY','REGION','CITY','NETWORK','DEVICE','VISITS','LAST SEEN'].forEach(x => hr.append(el('th','',x))); head.append(hr); const body = el('tbody');
      visitors.forEach(v => { const row = el('tr'); const gamertags=(v.associated_gamertags || []).join(', ') || '—'; const network=v.asn_org ? v.asn_org.replace(/^(AS\d+)\s+(.+)$/i, '$1 · $2') : '—'; [[v.ip,''],[gamertags,''],[v.country || 'Unknown','hide-mobile'],[v.region || 'Unknown','hide-mobile'],[v.city || 'Unknown','hide-mobile'],[network,'hide-mobile'],[v.device_category || 'Unknown','hide-mobile'],[v.visits,''],[relative(v.last_seen),'']].forEach(([value,className]) => row.append(el('td',className,value))); row.addEventListener('click',() => showVisitor(v.ip)); body.append(row); }); table.append(head,body); document.getElementById('visitors-content').replaceChildren(table);
  ...
  async function refresh() { document.getElementById('refresh').disabled = true; await Promise.allSettled([loadOverview(),loadChallenges(),loadVisitors(visitorPage)]); document.getElementById('refresh').disabled = false; }
