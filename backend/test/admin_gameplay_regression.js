...
async function query(sql, params = []) {
  ...
  if (normalized.startsWith('INSERT INTO analytics_events')) { capturedAnalytics={ ip:params[6], visitor_id:params[7], ref:params[8], utm_source:params[9], utm_medium:params[10], utm_campaign:params[11], utm_content:params[12], utm_term:params[13], event_type:params[14], section_name:params[15] }; return rows([{ id:99 }]); }
...
(async () => {
  ...
  try {
    ...
    if (!response.ok || capturedAnalytics?.ip !== '203.0.113.43' || capturedAnalytics?.visitor_id !== visitorId || capturedAnalytics?.ref !== null || capturedAnalytics?.utm_source !== null || capturedAnalytics?.event_type !== 'page_view') throw new Error('Direct analytics did not retain trusted identity/IP/event type or invented tracking values');
