...
async function query(sql, params = []) {
  ...
  if (normalized.startsWith('INSERT INTO analytics_events')) { capturedAnalytics={ user_agent:params[2], ip:params[6], visitor_id:params[7], ref:params[8], utm_source:params[9], utm_medium:params[10], utm_campaign:params[11], utm_content:params[12], utm_term:params[13], event_type:params[14], section_name:params[15], device_name:params[16], device_type:params[17], operating_system:params[18], operating_system_version:params[19], browser:params[20], browser_version:params[21] }; return rows([{ id:99 }]); }
  if (normalized.startsWith('SELECT EXISTS (SELECT 1 FROM analytics_events')) return rows([{ returning:hasPreviousPageView }]);
  if (normalized.startsWith('SELECT city FROM ip_geolocation_cache')) return rows([{ city:'Cancun' }]);
